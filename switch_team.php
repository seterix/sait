...
?>
    <div class="clearfix colelem" id="pu714"><!-- group -->
     <div class="clearfix grpelem" id="u714"><!-- group -->
	  ...
	  <form method='post' action="/count_price.php">
	  <p><b>выберите команду:</b></p>
	  <?
	  /*$ath = mysql_query("select * from users;");
		if($ath)
		{
			$author = mysql_fetch_array($ath);
			echo "</br>$author[balance]</br>";
			echo "$author[id]</br>";
			
		}*/
		
		require_once 'scripts/simple_html_dom.php'; 
		$html = file_get_html('http://dota2lounge.com/match?m='.$match_id);
		$num = 1;
		$mass = array();
		
		$match_status="";
		
		foreach($html->find('div[id=mf] b') as $element)
		{
			$match_status = $match_status." ".$element->innertext;
		}
		
		if(preg_match("/win/i", $match_status))
			echo "Матч закончен";
		else
		{
		...
		{ 
				...
				if(preg_match('#\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#', $link, $matches)) {
					...
					echo "<p><input type='radio' class='radio' name='team_num' value='$num'><img src='$image_url'/></input>";
					$num++;
				}	
